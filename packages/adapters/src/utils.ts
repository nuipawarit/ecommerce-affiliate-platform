...
export function extractShopeeProductId(url: string): { shopId: string; itemId: string } {
...
export function generateMockProduct(
  ...
  const price = Math.round(mockProduct.basePrice * (1 + (Math.random() > 0.5 ? priceVariance : -priceVariance)));

  return {
    title: mockProduct.title,
    imageUrl: mockProduct.imageUrl,
    storeName: `${mockProduct.storeName}${marketplace === 'lazada' ? ' - Lazada' : ' - Shopee'}`,
    price,
    marketplace,
    url
  };
...
export function searchMockProducts(
  ...
  return matchingProducts.map((product, index) => {
    const priceVariance = 0.05 + Math.random() * 0.15;
    const price = Math.round(product.basePrice * (1 + (Math.random() > 0.5 ? priceVariance : -priceVariance)));

    const urlBase = marketplace === 'lazada'
    ...
    return {
      ...
      storeName: `${product.storeName}${marketplace === 'lazada' ? ' - Lazada' : ' - Shopee'}`,
      price,
      marketplace,
      url: urlBase
    };
